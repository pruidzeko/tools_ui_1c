...
Функция ЭтоПеречислениеПоТипу(Тип) Экспорт
	Возврат УИ_ОбщегоНазначения.ЭтоПеречислениеПоТипу(Тип);
КонецФункции
...
Процедура ПодключитьВнешнююОбработкуКСеансу(АдресДвоичныхДанныхОбработки, ИмяОбработки = Неопределено) Экспорт
	УИ_ОбщегоНазначения.ПодключитьВнешнююОбработкуКСеансу(АдресДвоичныхДанныхОбработки, ИмяОбработки);
КонецПроцедуры

// Адрес двоичных данных общего макета.
// 
// Параметры:
...
// 
// Возвращаемое значение:
//  Строка - Адрес двоичных данных общего макета
Функция АдресДвоичныхДанныхОбщегоМакета(ИмяМакета, ИдентификаторФормы = Неопределено) Экспорт
	Если ИдентификаторФормы = Неопределено Тогда
		Возврат ПоместитьВоВременноеХранилище(ПолучитьОбщийМакет(ИмяМакета));
	Иначе
		Возврат ПоместитьВоВременноеХранилище(ПолучитьОбщийМакет(ИмяМакета), ИдентификаторФормы);
	КонецЕсли;
КонецФункции
...
// Получить проверку сложности длину паролей.
// 
// Параметры:
//  СтруктураПараметров - Структура 
// 			* МинДлинаПароля - Число - Минимальная длина пароля, сохраняем чтобы потом установить
// 			* ПроверкаСложностиПароля - Булево - Сохраняем параметр чтобы потом восстановить
// Возвращаемое значение: 
// Структура - возвращаем сложность и длину пароля:   
// 			* МинДлинаПароля - Число - Минимальная длина пароля, сохраняем чтобы потом установить
// 			* ПроверкаСложностиПароля - Булево - Сохраняем параметр чтобы потом восстановить 
Функция ПолучитьПроверкуСложностиДлинуПаролей(СтруктураПараметров) Экспорт
	СтруктураПараметров.Вставить("МинДлинаПароля", ПолучитьМинимальнуюДлинуПаролейПользователей());
	СтруктураПараметров.Вставить("ПроверкаСложностиПароля", ПолучитьПроверкуСложностиПаролейПользователей());
	Возврат СтруктураПараметров
КонецФункции


// Установить проверку сложности длину паролей.
// 
// Параметры:
//  СтруктураПараметров - Структура -, см. ПолучитьПроверкуСложностиДлинуПаролей 
//    	
//  СброситьПроверку - Булево - Сбросить проверку
Процедура УстановитьПроверкуСложностиДлинуПаролей(СтруктураПараметров, СброситьПроверку = Ложь) Экспорт
	Если СброситьПроверку Тогда
		Если СтруктураПараметров.МинДлинаПароля > 12 Тогда
			УстановитьМинимальнуюДлинуПаролейПользователей(0);
		КонецЕсли;
		Если СтруктураПараметров.ПроверкаСложностиПароля Тогда
			УстановитьПроверкуСложностиПаролейПользователей(Ложь);
		КонецЕсли;	
	Иначе
		Если СтруктураПараметров.МинДлинаПароля > 12 Тогда
			УстановитьМинимальнуюДлинуПаролейПользователей(СтруктураПараметров.МинДлинаПароля);
		КонецЕсли;
		Если СтруктураПараметров.ПроверкаСложностиПароля Тогда
			УстановитьПроверкуСложностиПаролейПользователей(СтруктураПараметров.ПроверкаСложностиПароля);
		КонецЕсли;		
	КонецЕсли;				
...
Функция ЗаписатьДанныеДляОтладкиВСправочник(ТипОбъектаОтладки, ДанныеДляОтладки, Наименование = "") Экспорт
	...
	Если ЕстьАктивнаяТранзакция И УИ_ОбщегоНазначенияКлиентСервер.ЭтоПортативнаяПоставка() Тогда
		Возврат ЗаписатьДанныеДляОтладкиВФайл(ТипОбъектаОтладки, ДанныеДляОтладки, Наименование);
	КонецЕсли;
...
// Записать данные для отладки в файл.
// 
// Параметры:
//  ТипОбъектаОтладки - Строка - Тип объекта отладки
//  ДанныеДляОтладки - Произвольный -Данные для отладки
...
//  Строка -СсылкаНаДанныеОтладки
// Результат выполнения сохранения данных отладки
Функция ЗаписатьДанныеДляОтладкиВФайл(ТипОбъектаОтладки, ДанныеДляОтладки, Наименование = "") Экспорт
	КаталогИнструментовНаСервере = УИ_ОбщегоНазначения.КаталогФайловДанныхОтладкиНаСервере();
	
	ИмяКаталога = Строка(Новый УникальныйИдентификатор);
	
	КаталогСохранения = УИ_ОбщегоНазначенияКлиентСервер.ОбъединитьПути(КаталогИнструментовНаСервере, ИмяКаталога);
	Попытка
	...
	ИмяФайлаДанных = УИ_ОбщегоНазначенияКлиентСервер.ОбъединитьПути(КаталогСохранения, "ДАННЫЕ.data");
	ИмяФайлаМетаданных = УИ_ОбщегоНазначенияКлиентСервер.ОбъединитьПути(КаталогСохранения, "МЕТАДАННЫЕ.json");
	
	МетаданныеДляСохранения = Новый Структура;
	МетаданныеДляСохранения.Вставить("Автор", ИмяПользователя());
	МетаданныеДляСохранения.Вставить("Дата", ТекущаяДата());
	МетаданныеДляСохранения.Вставить("ТипОбъектаОтладки", ТипОбъектаОтладки);
	МетаданныеДляСохранения.Вставить("Наименование", Наименование);
	
	УИ_ОбщегоНазначенияКлиентСервер.мЗаписатьJSONВФайл(ИмяФайлаМетаданных, МетаданныеДляСохранения);
	
	ЗначениеВФайл(ИмяФайлаДанных, ДанныеДляОтладки);
	
	Возврат "Запись выполнена успешно. Каталог сохранения на сервере "+ КаталогСохранения;
...
Функция АдресОбъектаОтладкиВоВременномХранилищеИзСистемногоХранилищаНастроек(КлючНастроек, Пользователь = Неопределено,
	ИдентификаторФормы = Неопределено) Экспорт
	КлючОбъектаДанныхОтладки=УИ_ОбщегоНазначенияКлиентСервер.КлючДанныхОбъектаДанныхОтладкиВХранилищеНастроек();
	НастройкиОтладки=УИ_ОбщегоНазначения.ХранилищеСистемныхНастроекЗагрузить(КлючОбъектаДанныхОтладки,
																			 КлючНастроек,
																			 ,
																			 ,
																			 Пользователь);
	...
	Возврат АдресОбъектаОтладки;
КонецФункции

Функция АдресОбъектаОтладкиВоВременномХранилищеИзКаталогаДанныхОтладки(КаталогДанныхОтладки, ИдентификаторФормы = Неопределено) Экспорт
	
	Файлы = НайтиФайлы(КаталогДанныхОтладки, "*.data");
	...
	НастройкиОтладки = ЗначениеИзФайла(ФайлДанных.ПолноеИмя);
...
Функция ПодготовленныеДанныеКонсолиДляЗаписиВФайл(ИмяКонсоли, ИмяФайла, АдресДанныхСохранения,
	...
	Если ИмяКонсоли = "РедактовHTML" Тогда
		МенеджерКонсоли = Обработки.УИ_РедакторHTML;
	КонецЕсли;
...
// Дополнить информацию для поддержки на сервере.
// 
// Параметры:
//  СтруктураИнформации - Структура - Структура информации
Процедура ДополнитьИнформациюДляПоддержкиНаСервере(СтруктураИнформации) Экспорт
	СтруктураИнформации.Вставить("Сервер", УИ_ОбщегоНазначенияКлиентСервер.ОписаниеОСДляТехподдержки());
	СтруктураИнформации.Вставить("ЭтоФайловаяБаза", УИ_ОбщегоНазначения.ИнформационнаяБазаФайловая());
	СтруктураИнформации.Вставить("ВерсияБСП", УИ_ОбщегоНазначения.ВерсияБСП());
	СтруктураИнформации.Вставить("Конфигурация", Новый Структура);
	СтруктураИнформации.Конфигурация.Вставить("Имя", Метаданные.Имя);
	СтруктураИнформации.Конфигурация.Вставить("РежимСовместимости", Строка(Метаданные.РежимСовместимости));
	СтруктураИнформации.Конфигурация.Вставить("Версия", Метаданные.Версия);
	СтруктураИнформации.Конфигурация.Вставить("ОсновнойРежимЗапуска", Строка(Метаданные.ОсновнойРежимЗапуска));
	СтруктураИнформации.Конфигурация.Вставить("РежимИспользованияМодальности",
											  Строка(Метаданные.РежимИспользованияМодальности));
	СтруктураИнформации.Конфигурация.Вставить("РежимИспользованияСинхронныхВызововРасширенийИВнешнихКомпонент",
											  Строка(Метаданные.РежимИспользованияСинхронныхВызововРасширенийИВнешнихКомпонент));
	СтруктураИнформации.Конфигурация.Вставить("РежимИспользованияСинхронныхВызововРасширенийПлатформыИВнешнихКомпонент",
											  Строка(Метаданные.РежимИспользованияСинхронныхВызововРасширенийПлатформыИВнешнихКомпонент));
	СтруктураИнформации.Конфигурация.Вставить("РежимИспользованияМодальности",
											  Строка(Метаданные.РежимИспользованияМодальности));
	СтруктураИнформации.Конфигурация.Вставить("РежимСовместимостиИнтерфейса",
											  Строка(Метаданные.РежимСовместимостиИнтерфейса));
	СтруктураИнформации.Конфигурация.Вставить("РежимУправленияБлокировкойДанных",
											  Строка(Метаданные.РежимУправленияБлокировкойДанных));
	СтруктураИнформации.Вставить("Расширения", Новый Массив);

	Для Каждого ТекРасширение Из РасширенияКонфигурации.Получить() Цикл
		СтруктураИнформации.Расширения.Добавить(""+ТекРасширение.Имя+"("+ТекРасширение.Версия+")");
	КонецЦикла;
КонецПроцедуры
